...
Student::Student():IOInterface() //디폴트 생성자 
{
   m_name = "";
   m_hakbun = 0;
   m_subnum = 0;
...
Student::Student(string name, int hakbun, int subnum, Subject* sub):IOInterface(name) // 인자있는 생성자_깊은 복사 해야함
{
   m_name = name;
   m_hakbun = hakbun;
   m_subnum = subnum;
...
Student::Student(const Student& std):IOInterface(std) //복사생성자
{
...
void Student::Modify() { //학생의 정보를 수정하는 함수
...
string Student::GetName()const // 접근자함수 : 클래스 밖에서 멤버변수를 읽을 수 있게함.
{
   return m_name;
}
