...
Subject::Subject():IOInterface() // 디폴트 생성자 구현부
{
   m_name = "";
   m_hakjum = 0;
   m_grade = "";
...
Subject::Subject(string name, int hakjum, string grade):IOInterface(name) // 인자있는 생성자
{
   m_name = name;
   m_hakjum = hakjum;
   m_grade = grade;
...
string Subject::GetName()const // 접근자함수 : m_name을 클래스 밖에서 읽을 수 있게함.
{
   return m_name;
}
